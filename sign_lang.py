...
class Sign():
    ...
    def letter_I:
        ...
        index_finger_mcp = self.xyz[5]
        thumb_ip = self.xyz[3]
        if and palm(ring_tip) and palm(middle_tip) and distance(index_finger_pip, index_finger_mcp) < self.accuracy and distance(index_finger_mcp, thumb_ip) < self.accuracy:
            return True
        else
        return False
