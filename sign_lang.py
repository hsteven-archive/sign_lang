...
class Sign():
    ...
    def letter_A(self):
        ...
        if self.palm(pinky_tip) and self.palm(ring_tip) and self.palm(middle_tip) and distance(index_finger_pip, index_finger_mcp) < self.accuracy:
            if distance(index_finger_pip, thumb_ip) < self.accuracy and distance(index_tip, thumb_ip) > self.accuracy:
                return True
        return False
    ...
    def letter_I(self):
        ...
        if self.palm(ring_tip) and self.palm(middle_tip) and distance(index_finger_pip, index_finger_mcp) < self.accuracy:
            if  distance(index_finger_dip, thumb_ip) < self.accuracy and distance(pinky_tip, ring_tip) > self.accuracy:
                return True
        return False
    ...
    def letter_U(self):
        ...
        if distance(ring_tip, thumb_tip) < self.accuracy and distance(index_tip, middle_tip) < self.accuracy and distance(pinky_tip, ring_tip) < self.accuracy:
            if self.palm_direction(wrist, middle_pip, index_pip)[1] != "down":
                return True
        return False
    ...
    def letter_S(self):
        ...
        middle_pip = self.xyz[10]
        index_pip = self.xyz[6]
        if distance(ring_pip, thumb_tip) < self.accuracy and distance(thumb_mcp, index_tip) < self.accuracy and self.palm(ring_tip) and self.palm(middle_tip) and distance(index_pip, middle_pip) < self.accuracy:
            return True
        return False
    ...
    def letter_T(self):
        ...
        if distance(index_tip, thumb_ip) < self.accuracy:
            if distance(pinky_tip, ring_tip) < self.accuracy:
                if self.palm(middle_tip) and self.palm(ring_tip):
                    if distance(thumb_tip, middle_pip) < self.accuracy:
                        return True
        return False
    ...
    def letter_R(self):
        ...
        middle_tip = self.xyz[12]
        index_tip = self.xyz[8]
        if self.palm(ring_tip) and distance(index_pip, middle_pip) < self.accuracy and distance(thumb_tip, pinky_dip) < self.accuracy and distance(middle_tip, index_tip) > 0.01:
            return True
        else:
            return False
    ...
    def letter_X(self):
        ...
        index_tip = self.xyz[8]
        index_mcp = self.xyz[5]
        if self.palm(ring_tip) and self.palm(middle_tip) and distance(pinky_pip, ring_pip) < self.accuracy and distance(thumb_tip, ring_dip) < self.accuracy:
            if distance(index_pip, index_dip) < self.accuracy and distance(index_tip, index_dip) < self.accuracy:
                return True
        else:
            return False
